...
summary: Time-attack racing stripped to the clock as sole opponent, instant restart as the practice loop, and a medal ladder that fits one track to every skill.
...
practice loop: try a braking point, miss, restart, adjust, repeat. Failure never
compounds into frustration because it never costs anything. The retry *is* the
practice, and the practice *is* the fun.
...
and an expert play the identical track toward different, always-visible goals.
No difficulty menu, no rubber-band: the ramp is the medal you reach for, and one
...
- **Make restart free and instant.** Bind retry to one key, respawn at the start
  (or last checkpoint) with no load. Cheap failure is the license for a hard
  line, and the retry loop teaches while it punishes.
...
- **The 3D stadium, loops, and stunt spectacle.** Pure coat of paint. The loop is
  dimension-agnostic: it works top-down, side-on, or first-person.
- **The specific medal names and colors.** *A tiered ladder of target times* is
  ...
  same DNA.
- **The online track-of-the-day and global leaderboards.** A social layer over
  the loop, not the loop. The single-player time attack is fun with zero
  connectivity.
...
- [[genre-racing]]: its canonical anchor; the speed/line tradeoff and the honest
  clock live there.
