...
- `/workshop` — act on human playtests: the `hayao-workshop` MCP server (.mcp.json)
  reads `.workshop/` sessions, replays any tick headlessly (`inspect_moment`),
  and reports hesitations/deaths/futile verbs (`get_playtest_report`). Humans
  play + tune declared knobs at `/workshop/` on the dev server. `docs/WORKSHOP.md`.
