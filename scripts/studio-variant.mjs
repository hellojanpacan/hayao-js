// Build a WORKTREE variant for Studio A/B: a git ref built to static files the
// dev server serves under /__studio/variants/<name>/. Unlike module variants
...
//
// Run:  node scripts/studio-variant.mjs <name> <git-ref>
//   e.g. node scripts/studio-variant.mjs before-jump-fix HEAD~1
//
// The worktree is temporary (removed after the build); .studio/variants.json
// records name → {commit, ref, builtAt} for the Studio UI and the MCP sidecar.
...
if (!name || !ref || !/^[a-z0-9][a-z0-9._-]*$/i.test(name)) {
  console.error('usage: node scripts/studio-variant.mjs <name> <git-ref>   (name: [a-z0-9._-])');
  process.exit(1);
}

const root = process.cwd();
const worktree = join(root, '.studio', 'worktrees', name);
const outDir = join(root, '.studio', 'variants', name);
...
mkdirSync(join(root, '.studio', 'worktrees'), { recursive: true });
...
try {
  ...
  execFileSync('npx', ['vite', 'build', '--outDir', outDir, '--emptyOutDir'], {
    cwd: worktree,
    stdio: 'inherit',
    env: { ...process.env, BASE_PATH: `/__studio/variants/${name}/` },
  });

  const registryPath = join(root, '.studio', 'variants.json');
  ...
  console.log(`✓ built .studio/variants/${name} and registered in variants.json`);
} finally {
  try {
    git('worktree', 'remove', '--force', worktree);
  } catch {
