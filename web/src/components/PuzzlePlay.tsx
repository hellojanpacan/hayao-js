...
/**
 * The live "Mobile Puzzle" — Lumen — running on this page. The engine + game
 * chunk is dynamically imported only when the visitor taps Play. The result
 * overlay mounts into `mount` (setOverlayHost), so the night CSS vars on the
 * wrapper theme it to match the game's navy ground.
 */
...
export default function PuzzlePlay() {
  ...
  async function onPlay() {
    ...
    setLoading(true);
    const { createLumen } = await import("../play/lumen");
    if (handleRef.current) return; // double-tap guard
    handleRef.current = createLumen(mountRef.current);
    setLoading(false);
    setStarted(true);
    wrapRef.current?.focus();
  }
  ...
  return (
    <div className="flex flex-col items-center">
      <div
        ...
        style={NIGHT_VARS}
        className="relative aspect-[9/16] w-full max-w-[380px] overflow-hidden rounded-[2rem] border border-hair bg-[#141a30] shadow-[0_24px_60px_-24px_rgba(41,51,92,0.5)] outline-none focus-visible:ring-2 focus-visible:ring-orange"
        aria-label="Lumen — drag a spark onto the board to light every prism"
      >
        ...
        {!started && (
          ...
          >
            ...
            <span className="font-display text-[0.95rem] font-semibold text-white">{loading ? "Loading engine…" : "Tap to play"}</span>
            <span className="font-body text-[0.8rem] font-light text-white/60">Drag a spark to light the board</span>
          </button>
        )}
        {started && paused && (
          <button
          ...
          >
            <span className="rounded-full bg-panel px-5 py-2.5 font-display text-[0.9rem] font-semibold text-ink shadow-lg">Paused — tap to resume</span>
          </button>
        )}
      </div>
