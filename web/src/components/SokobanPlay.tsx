...
/**
 * The live Sokoban — the repo's reference example — running on this page. The
 * engine + game chunk is dynamically imported only when the visitor taps Play.
 * The result overlay mounts into `mount`, so the night CSS vars on the wrapper
 * theme it to match the game's navy ground.
 */

// Night overlay theme for the in-game screens (mount inside the frame).
...
export default function SokobanPlay() {
  ...
  async function onPlay() {
    ...
    setLoading(true);
    const { createSokoban } = await import("../play/sokoban");
    if (handleRef.current) return; // double-tap guard
    handleRef.current = createSokoban(mountRef.current);
    setLoading(false);
    setStarted(true);
    wrapRef.current?.focus();
  }
  ...
  return (
    <div className="flex flex-col items-center">
      <div
        ...
        style={NIGHT_VARS}
        className="relative aspect-[4/3] w-full overflow-hidden rounded-[1.5rem] border border-hair bg-[#141a30] shadow-[0_24px_60px_-24px_rgba(41,51,92,0.5)] outline-none focus-visible:ring-2 focus-visible:ring-orange"
        aria-label="Sokoban — push every box onto a goal; arrows move, U undoes"
      >
        ...
        {!started && (
          ...
          >
            ...
            <span className="font-body text-[0.8rem] font-light text-white/60">Arrows push · U undoes · every level solver-proven</span>
          </button>
        )}
        {started && paused && (
          <button
