...
| **Hayabox** | `src/audio/hayabox.ts` | The house LoopDeck: six courtiers (one per Regalia job), ≥2 moods each, every note in A minor pentatonic so ANY subset of awake stems harmonizes — a property of the data, proven in `hayabox.test.ts` (lintDeck + zero out-of-key + rendered-tutti gates). The Fool (drums) plays the layered-snare kick-snare kit; the Chancellor (bass) carries a `smoothBass` "Silk" mood — both authored with the same `stepPattern`/voices above. Live on the site at `/play/hayabox`. |
